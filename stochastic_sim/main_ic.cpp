...
void usage(char *argv[]){
  ...
  printf("Usage: %s [simulation time] [dim ensemble] [input rate profile] [# rates (length of input)] [L] [output prefix] [input initial density profile]\n\n" , argv[0]);
}
...
//     double * q_rates;
    ...
    simul_t = atof(argv[1]);  // 300
    DIM_ENSEMBLE = atoi(argv[2]);
    
    n_of_rates = atoi(argv[4]);
    L = atof(argv[5]); // 100
    
    dim_chain = n_of_rates - 1;
    a = L / dim_chain;
    rates = new double [n_of_rates];
    initial_condition = new double [dim_chain];
...
//     q_rates = new double [n_of_rates];
    ...
    snprintf(densities_file_name, 120, "%sprofiles_time=%s_ensem=%s_N=%d_L=%s.txt", argv[7], argv[1], argv[2], dim_chain, argv[5]);
    densities_file = fopen(densities_file_name, "w");

    rates_file = fopen(argv[3], "r");
    if (rates_file == NULL) {
...
/////////////////////////////    
    initial_condition_file = fopen(argv[6], "r");
    if (initial_condition_file == NULL) {
        perror("Error while opening the file.\n");
        exit(EXIT_FAILURE);
    }
    for (i = 0; i < dim_chain; i++){
        status = fscanf(initial_condition_file, "%lf\n", &initial_condition[i]);
    }
...
//     }
    ...
    for (i=0; i<DIM_ENSEMBLE; i++) {
        ...
        for (j=0; j<dim_chain; j++){
            system[i].node[j] = gsl_ran_bernoulli(r, initial_condition[j]);
